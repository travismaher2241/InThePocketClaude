...
const txtPath = "C:\\Users\\travi\\.gemini\\antigravity\\brain\\12eef90f-48b2-4612-872d-00c9ff24dbba\\scratch\\extracted_ch11_new.txt";
const outChapterPath = "C:\\TCLS Projects\\CoachCore\\data\\generated\\ch11-team-defence.json";
...
for (let i = 0; i < lines.length; i++) {
  ...
  const headerMatch = line.match(/^TD-(\d{3})\s+[–-]\s+(.+)$/i);
...
console.log(`Extracted ${drills.length} unique main drill records from new Chapter 11 file.`);
...
const structuredDrills = drills.map(d => {
  ...
  return {
    drillId: d.drillId,
    title: d.title,
    category: rf["Category"] || "Team Defence",
...
console.log(`Saved Chapter 11 dataset to ${outChapterPath}`);
...
masterDrills = masterDrills.filter(d => !d.drillId.startsWith('TD-')).concat(structuredDrills);
