...
const txtPath = path.join("C:", "Users", "travi", ".gemini", "antigravity", "brain", "12eef90f-48b2-4612-872d-00c9ff24dbba", "scratch", "extracted_ch3.txt");
const outChapterPath = "C:\\TCLS Projects\\CoachCore\\data\\generated\\ch3-marking.json";
...
for (let i = 0; i < lines.length; i++) {
  ...
  const headerMatch = line.match(/^MK-(\d{3})\s+[–-]\s+(.+)$/i);
...
console.log(`Deduplicated and extracted ${drills.length} unique main drill records for Chapter 3 (MK-001 to MK-120).`);
...
console.log(`Saved Chapter 3 dataset to ${outChapterPath}`);
...
masterDrills = masterDrills.filter(d => !d.drillId.startsWith('MK-')).concat(structuredDrills);
