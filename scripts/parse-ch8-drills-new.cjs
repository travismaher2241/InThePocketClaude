...
const txtPath = "C:\\Users\\travi\\.gemini\\antigravity\\brain\\12eef90f-48b2-4612-872d-00c9ff24dbba\\scratch\\extracted_ch8_new.txt";
const outChapterPath = "C:\\TCLS Projects\\CoachCore\\data\\generated\\ch8-evasion-agility-and-movement.json";
...
for (let i = 0; i < lines.length; i++) {
  ...
  const headerMatch = line.match(/^EA-(\d{3})\s+[–-]\s+(.+)$/i);
  ...
  if (headerMatch) {
    ...
    if (isMainRecord) {
      ...
      currentDrill = {
        ...
        title: title,
        chapter: 8,
        chapterName: "Evasion, Agility and Movement",
        rawFields: {}
      };
      currentField = null;
      fieldContent = [];
...
console.log(`Extracted ${drills.length} unique main drill records from new Chapter 8 file.`);
...
const structuredDrills = drills.map(d => {
  ...
  return {
    drillId: d.drillId,
    title: d.title,
    category: rf["Category"] || "Evasion, Agility and Movement",
...
console.log(`Saved Chapter 8 dataset to ${outChapterPath}`);
...
masterDrills = masterDrills.filter(d => !d.drillId.startsWith('EA-')).concat(structuredDrills);
