...
const txtPath = "C:\\Users\\travi\\.gemini\\antigravity\\brain\\12eef90f-48b2-4612-872d-00c9ff24dbba\\scratch\\extracted_ch5_new.txt";
const outChapterPath = "C:\\TCLS Projects\\CoachCore\\data\\generated\\ch5-tackling-and-pressure.json";
...
for (let i = 0; i < lines.length; i++) {
  ...
  const headerMatch = line.match(/^TK-(\d{3})\s+[–-]\s+(.+)$/i);
  ...
  if (headerMatch) {
    ...
    if (isMainRecord) {
      ...
      currentDrill = {
        ...
        title: title,
        chapter: 5,
        chapterName: "Tackling and Pressure",
        rawFields: {}
      };
      currentField = null;
      fieldContent = [];
...
console.log(`Extracted ${drills.length} unique main drill records from new Chapter 5 file.`);
...
const structuredDrills = drills.map(d => {
  ...
  return {
    drillId: d.drillId,
    title: d.title,
    category: rf["Category"] || "Tackling and Pressure",
...
console.log(`Saved Chapter 5 dataset to ${outChapterPath}`);
...
masterDrills = masterDrills.filter(d => !d.drillId.startsWith('TK-')).concat(structuredDrills);
