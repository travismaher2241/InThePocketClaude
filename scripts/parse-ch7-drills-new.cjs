...
const txtPath = "C:\\Users\\travi\\.gemini\\antigravity\\brain\\12eef90f-48b2-4612-872d-00c9ff24dbba\\scratch\\extracted_ch7_new.txt";
const outChapterPath = "C:\\TCLS Projects\\CoachCore\\data\\generated\\ch7-ruck-and-stoppage-craft.json";
...
for (let i = 0; i < lines.length; i++) {
  ...
  const headerMatch = line.match(/^RK-(\d{3})\s+[–-]\s+(.+)$/i);
  ...
  if (headerMatch) {
    ...
    if (isMainRecord) {
      ...
      currentDrill = {
        ...
        title: title,
        chapter: 7,
        chapterName: "Ruck and Stoppage Craft",
        rawFields: {}
      };
      currentField = null;
      fieldContent = [];
...
console.log(`Extracted ${drills.length} unique main drill records from new Chapter 7 file.`);
...
const structuredDrills = drills.map(d => {
  ...
  return {
    drillId: d.drillId,
    title: d.title,
    category: rf["Category"] || "Ruck and Stoppage Craft",
...
console.log(`Saved Chapter 7 dataset to ${outChapterPath}`);
...
masterDrills = masterDrills.filter(d => !d.drillId.startsWith('RK-')).concat(structuredDrills);
