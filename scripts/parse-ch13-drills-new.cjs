...
const txtPath = "C:\\Users\\travi\\.gemini\\antigravity\\brain\\12eef90f-48b2-4612-872d-00c9ff24dbba\\scratch\\extracted_ch13_new.txt";
const outChapterPath = "C:\\TCLS Projects\\CoachCore\\data\\generated\\ch13-conditioning-with-football.json";
...
for (let i = 0; i < lines.length; i++) {
  ...
  const headerMatch = line.match(/^CF-(\d{3})\s+[–-]\s+(.+)$/i);
  ...
  if (headerMatch) {
    ...
    if (isMainRecord) {
      ...
      currentDrill = {
        ...
        title: title,
        chapter: 13,
        chapterName: "Conditioning with Football",
        rawFields: {}
      };
      currentField = null;
      fieldContent = [];
...
console.log(`Extracted ${drills.length} unique main drill records from new Chapter 13 file.`);
...
const structuredDrills = drills.map(d => {
  ...
  return {
    drillId: d.drillId,
    title: d.title,
    category: rf["Category"] || "Conditioning with Football",
...
console.log(`Saved Chapter 13 dataset to ${outChapterPath}`);
...
masterDrills = masterDrills.filter(d => !d.drillId.startsWith('CF-')).concat(structuredDrills);
