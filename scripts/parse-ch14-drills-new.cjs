...
const txtPath = "C:\\Users\\travi\\.gemini\\antigravity\\brain\\12eef90f-48b2-4612-872d-00c9ff24dbba\\scratch\\extracted_ch14_new.txt";
const outChapterPath = "C:\\TCLS Projects\\CoachCore\\data\\generated\\ch14-small-sided-games.json";
...
for (let i = 0; i < lines.length; i++) {
  ...
  const headerMatch = line.match(/^SG-(\d{3})\s+[–-]\s+(.+)$/i);
...
console.log(`Extracted ${drills.length} unique main drill records from new Chapter 14 file.`);
...
const structuredDrills = drills.map(d => {
  ...
  return {
    drillId: d.drillId,
    title: d.title,
    category: rf["Category"] || "Small-Sided Games",
...
console.log(`Saved Chapter 14 dataset to ${outChapterPath}`);
...
masterDrills = masterDrills.filter(d => !d.drillId.startsWith('SG-')).concat(structuredDrills);
