...
const txtPath = "C:\\Users\\travi\\.gemini\\antigravity\\brain\\12eef90f-48b2-4612-872d-00c9ff24dbba\\scratch\\extracted_ch2.txt";
const outChapterPath = "C:\\TCLS Projects\\CoachCore\\data\\generated\\ch2-handballing.json";
...
for (let i = 0; i < lines.length; i++) {
  ...
  const headerMatch = line.match(/^HB-(\d{3})\s+[–-]\s+(.+)$/i);
...
console.log(`Deduplicated and extracted ${drills.length} unique main drill records for Chapter 2 (HB-001 to HB-120).`);
...
const structuredDrills = drills.map(d => {
  ...
  return {
    drillId: d.drillId,
    title: d.title,
    category: rf["Category"] || "Handballing",
...
console.log(`Saved Chapter 2 dataset to ${outChapterPath}`);
...
masterDrills = masterDrills.filter(d => !d.drillId.startsWith('HB-')).concat(structuredDrills);
