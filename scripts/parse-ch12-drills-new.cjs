...
const txtPath = "C:\\Users\\travi\\.gemini\\antigravity\\brain\\12eef90f-48b2-4612-872d-00c9ff24dbba\\scratch\\extracted_ch12_new.txt";
const outChapterPath = "C:\\TCLS Projects\\CoachCore\\data\\generated\\ch12-transition.json";
...
for (let i = 0; i < lines.length; i++) {
  ...
  const headerMatch = line.match(/^TR-(\d{3})\s+[–-]\s+(.+)$/i);
...
console.log(`Extracted ${drills.length} unique main drill records from new Chapter 12 file.`);
...
const structuredDrills = drills.map(d => {
  ...
  return {
    drillId: d.drillId,
    title: d.title,
    category: rf["Category"] || "Transition",
...
console.log(`Saved Chapter 12 dataset to ${outChapterPath}`);
...
masterDrills = masterDrills.filter(d => !d.drillId.startsWith('TR-')).concat(structuredDrills);
