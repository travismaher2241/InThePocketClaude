...
const txtPath = path.join("C:", "Users", "travi", ".gemini", "antigravity", "brain", "12eef90f-48b2-4612-872d-00c9ff24dbba", "scratch", "extracted_ch4.txt");
const outChapterPath = "C:\\TCLS Projects\\CoachCore\\data\\generated\\ch4-ground-balls.json";
...
for (let i = 0; i < lines.length; i++) {
  ...
  const headerMatch = line.match(/^GB-(\d{3})\s+[–-]\s+(.+)$/i);
...
console.log(`Deduplicated and extracted ${drills.length} unique main drill records for Chapter 4 (GB-001 to GB-120).`);
...
const structuredDrills = drills.map(d => {
  ...
  return {
    drillId: d.drillId,
    title: d.title,
    category: rf["Category"] || "Ground Balls",
...
console.log(`Saved Chapter 4 dataset to ${outChapterPath}`);

// Update master drill dataset
let masterDrills = [];
...
masterDrills = masterDrills.filter(d => !d.drillId.startsWith('GB-')).concat(structuredDrills);
