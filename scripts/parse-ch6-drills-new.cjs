...
const txtPath = "C:\\Users\\travi\\.gemini\\antigravity\\brain\\12eef90f-48b2-4612-872d-00c9ff24dbba\\scratch\\extracted_ch6_new.txt";
const outChapterPath = "C:\\TCLS Projects\\CoachCore\\data\\generated\\ch6-spoiling-and-aerial-defence.json";
...
for (let i = 0; i < lines.length; i++) {
  ...
  const headerMatch = line.match(/^SP-(\d{3})\s+[–-]\s+(.+)$/i);
  ...
  if (headerMatch) {
    ...
    if (isMainRecord) {
      ...
      currentDrill = {
        ...
        title: title,
        chapter: 6,
        chapterName: "Spoiling and Aerial Defence",
        rawFields: {}
      };
      currentField = null;
      fieldContent = [];
...
console.log(`Extracted ${drills.length} unique main drill records from new Chapter 6 file.`);
...
const structuredDrills = drills.map(d => {
  ...
  return {
    drillId: d.drillId,
    title: d.title,
    category: rf["Category"] || "Spoiling and Aerial Defence",
...
console.log(`Saved Chapter 6 dataset to ${outChapterPath}`);
...
masterDrills = masterDrills.filter(d => !d.drillId.startsWith('SP-')).concat(structuredDrills);
