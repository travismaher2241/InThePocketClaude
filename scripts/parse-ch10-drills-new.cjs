...
const txtPath = "C:\\Users\\travi\\.gemini\\antigravity\\brain\\12eef90f-48b2-4612-872d-00c9ff24dbba\\scratch\\extracted_ch10_new.txt";
const outChapterPath = "C:\\TCLS Projects\\CoachCore\\data\\generated\\ch10-team-offence.json";
...
for (let i = 0; i < lines.length; i++) {
  ...
  const headerMatch = line.match(/^TO-(\d{3})\s+[–-]\s+(.+)$/i);
...
console.log(`Extracted ${drills.length} unique main drill records from new Chapter 10 file.`);
...
const structuredDrills = drills.map(d => {
  ...
  return {
    drillId: d.drillId,
    title: d.title,
    category: rf["Category"] || "Team Offence",
...
console.log(`Saved Chapter 10 dataset to ${outChapterPath}`);
...
masterDrills = masterDrills.filter(d => !d.drillId.startsWith('TO-')).concat(structuredDrills);
