...
const txtPath = "C:\\Users\\travi\\.gemini\\antigravity\\brain\\12eef90f-48b2-4612-872d-00c9ff24dbba\\scratch\\extracted_ch9_new.txt";
const outChapterPath = "C:\\TCLS Projects\\CoachCore\\data\\generated\\ch9-decision-making.json";
...
for (let i = 0; i < lines.length; i++) {
  ...
  const headerMatch = line.match(/^DM-(\d{3})\s+[–-]\s+(.+)$/i);
...
console.log(`Extracted ${drills.length} unique main drill records from new Chapter 9 file.`);
...
const structuredDrills = drills.map(d => {
  ...
  return {
    drillId: d.drillId,
    title: d.title,
    category: rf["Category"] || "Decision Making",
...
console.log(`Saved Chapter 9 dataset to ${outChapterPath}`);
...
masterDrills = masterDrills.filter(d => !d.drillId.startsWith('DM-')).concat(structuredDrills);
